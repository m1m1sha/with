...
impl<B: AsRef<[u8]>> BroadcastPacket<B> {
    ...
    pub fn data(&self) -> Result<&[u8]> {
        ...
        if start > self.buffer.as_ref().len() {
            Err(Error::new(ErrorKind::InvalidData, "InvalidData"))
        } else {
...
impl<B: AsRef<[u8]> + AsMut<[u8]>> BroadcastPacket<B> {
    pub fn set_address(&mut self, addr: &[Ipv4Addr]) -> Result<()> {
        ...
        if buf.len() < 1 + addr.len() * 4 || addr.len() > u8::MAX as usize {
            Err(Error::new(ErrorKind::InvalidData, "addr invalid data"))
        } else {
    ...
    pub fn set_data(&mut self, data: &[u8]) -> Result<()> {
        ...
        if start >= buf.len() || start + data.len() != buf.len() {
            return Err(Error::new(ErrorKind::InvalidData, "data invalid data"));
        }
