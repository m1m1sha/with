...
impl<B: AsRef<[u8]>> SecretBody<B> {
    pub fn new(buffer: B, exist_finger: bool) -> Result<SecretBody<B>> {
...
impl<B: AsRef<[u8]> + AsMut<[u8]>> SecretBody<B> {
    ...
    pub fn set_tag(&mut self, tag: &[u8]) -> Result<()> {
        if tag.len() != 16 {
            return Err(Error::new(ErrorKind::InvalidData, "tag.len != 16"));
        }
        let mut end = self.buffer.as_ref().len();
        if self.exist_finger {
            end -= 12;
    ...
    pub fn set_finger(&mut self, finger: &[u8]) -> Result<()> {
        if self.exist_finger {
            if finger.len() != 12 {
                return Err(Error::new(ErrorKind::InvalidData, "finger.len != 12"));
            }
            let end = self.buffer.as_ref().len();
            self.buffer.as_mut()[end - 12..end].copy_from_slice(finger);
            Ok(())
        } else {
            Err(Error::new(ErrorKind::InvalidData, "not exist finger"))
        }
    }
...
impl<B: AsRef<[u8]>> AesCbcSecretBody<B> {
    pub fn new(buffer: B, exist_finger: bool) -> Result<AesCbcSecretBody<B>> {
        let len = buffer.as_ref().len();
...
impl<B: AsRef<[u8]> + AsMut<[u8]>> AesCbcSecretBody<B> {
    ...
    pub fn set_finger(&mut self, finger: &[u8]) -> Result<()> {
        if self.exist_finger {
            ...
            Err(Error::new(ErrorKind::InvalidData, "cbc not exist finger"))
        }
    }
    pub fn en_body_mut(&mut self) -> &mut [u8] {
        let mut end = self.buffer.as_ref().len();
...
impl<B: AsRef<[u8]>> RsaSecretBody<B> {
    pub fn new(buffer: B) -> Result<RsaSecretBody<B>> {
        let len = buffer.as_ref().len();
        // 不能大于udp最大载荷长度
        if len < 32 || len > 65535 - 20 - 8 - 12 {
            return Err(Error::new(ErrorKind::InvalidData, "length overflow"));
        }
...
impl<B: AsRef<[u8]> + AsMut<[u8]>> RsaSecretBody<B> {
    pub fn set_random(&mut self, random: &[u8]) -> Result<()> {
        if random.len() != 16 {
            return Err(Error::new(ErrorKind::InvalidData, "random.len != 16"));
        }
        let end = self.buffer.as_ref().len() - 16;
    ...
    pub fn set_finger(&mut self, finger: &[u8]) -> Result<()> {
        if finger.len() != 16 {
            return Err(Error::new(ErrorKind::InvalidData, "finger.len != 16"));
        }
        let end = self.buffer.as_ref().len();
