...
use protocol::control::PingPacket;
use protocol::{control, NetPacket, Protocol, Version, MAX_TTL};
...
fn heartbeat0(
    ...
    match heartbeat_packet_server(device_list, server_cipher, src_ip, gateway_ip) {
        Ok(net_packet) => {
            ...
            {
                tracing::warn!("heartbeat err={:?}", e)
            } else {
                is_send_gateway = true
            }
        }
        Err(e) => {
            tracing::error!("heartbeat_packet err={:?}", e);
        }
    }
    ...
    for peer in &peer_list {
        ...
        if context.route_table.route_one(&peer.virtual_ip).is_none() {
            ...
            if let Err(e) = context.send_default(net_packet.buffer(), current_device.connect_server)
            {
                tracing::error!("heartbeat_packet send_default err={:?}", e);
            }
        }
    }
}
...
fn heartbeat_packet(
    ...
    net_packet.set_transport_protocol(control::Protocol::Ping.into());
    net_packet.first_set_ttl(MAX_TTL);
    net_packet.set_source(src);
    net_packet.set_destination(dest);
    let mut ping = PingPacket::new(net_packet.payload_mut())?;
    ping.set_time(crate::handler::now_time() as u16);
    Ok(net_packet)
}
