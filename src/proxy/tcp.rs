...
impl TcpProxy {
    pub fn new(stoper: Stoper) -> io::Result<Self> {
        ...
        let port = tcp_listener.local_addr()?.port();
        {
            ...
            thread::spawn(move || {
                if let Err(e) = tcp_proxy(tcp_listener, nat_map, stoper) {
                    tracing::warn!("tcp_proxy:{:?}", e);
                }
            });
        }
...
fn tcp_proxy(
    ...
    let _worker = stoper.add_listener("tcp_proxy".into(), move || {
        if let Err(e) = stop.wake() {
            tracing::warn!("stop tcp_proxy:{:?}", e);
        }
    })?;
    loop {
                    ...
                    match tcp_connect(addr.port(), dest_addr.into()) {
                        Ok(mut dest_stream) => {
                            ...
                            if let Err(e) = registry.register(
                                ...
                                tracing::error!("register src_stream:{:?}", e);
                                continue;
                            }
                            if let Err(e) = registry.register(
                                &mut dest_stream,
                                ...
                                tracing::error!("register dest_stream:{:?}", e);
                                continue;
                            }
                            tcp_map.insert(
                                src_fd,
                        ...
                        Err(e) => {
                            tracing::error!("connect:{:?} {}->{}", e, addr, dest_addr);
                        }
                    }
                }
            }
...
fn tcp_connect(src_port: u16, addr: SocketAddr) -> io::Result<TcpStream> {
    ...
    if let Err(e) = socket.set_tcp_keepalive(
        ...
        tracing::warn!("set_tcp_keepalive err {:?}", e);
    }
