...
use packet::icmp::{self, HeaderOther};
...
use cipher::Cipher;
use protocol::{NetPacket, Version, MAX_TTL};
...
fn icmp_proxy(
    ...
    let _worker = stoper.add_listener("icmp_proxy".into(), move || {
        if let Err(e) = stop.wake() {
            tracing::warn!("stop icmp_proxy:{:?}", e);
        }
    })?;
