...
impl UdpProxy {
    pub fn new(scheduler: Scheduler, stoper: Stoper) -> io::Result<Self> {
        ...
        let port = udp.local_addr()?.port();
        {
            ...
            thread::spawn(move || {
                if let Err(e) = udp_proxy(udp, nat_map, scheduler, stoper) {
                    tracing::warn!("udp_proxy:{:?}", e);
                }
            });
        }
...
fn udp_proxy(
    ...
    let _worker = stoper.add_listener("udp_proxy".into(), move || {
        if let Err(e) = stop.wake() {
            tracing::warn!("stop udp_proxy:{:?}", e);
        }
    })?;
    loop {
        let mut check = false;
            ...
            match udp_connect(src_addr.port(), dest_addr.into()) {
                Ok((token_val, mut dest_udp)) => {
                    ...
                    if let Err(e) = registry.register(&mut dest_udp, token, Interest::READABLE) {
                        tracing::error!("register失败:{:?},addr={:?}", e, dest_addr);
                        continue;
                    }
