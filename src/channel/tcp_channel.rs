    ...
    H: RecvChannelHandler,
{
    ...
    let worker = {
        ...
        stoper.add_listener("tcp_listen".into(), move || {
...
{
    let (tcp_sender, tcp_receiver) = sync_channel(64);
    let write_waker = init_writable_handler(tcp_receiver, stoper.clone(), context.clone())?;
    poll.registry()
...
fn init_writable_handler(
    ...
    let worker = {
        ...
        stoper.add_listener("tcp_writable_handler".into(), move || {
                    ...
                    if writable_notify.is_add_socket() {
                        ...
                        while let Ok((mut stream, token, addr, init_buf)) = receiver.try_recv() {
                            if let Err(e) = stream.set_nodelay(true) {
                                tracing::warn!("set_nodelay err={:?}", e);
                            }
                            if let Err(e) =
                                poll.registry()
                                    .register(&mut stream, token, Interest::WRITABLE)
                            {
                                tracing::warn!("registry err={:?}", e);
                                continue;
                            }
            ...
            match tcp_sender.try_send((TcpStream::from_std(tcp_writer), token, addr, init_buf)) {
                Ok(_) => {
                    if let Err(e) = write_waker.add_socket() {
                        tracing::error!("write_waker,err={:?},addr={:?}", e, addr);
                        return Ok(());
                    }
                }
                Err(e) => {
