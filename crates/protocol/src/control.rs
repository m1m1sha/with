...
impl<B: AsRef<[u8]>> ControlPacket<B> {
    pub fn new(protocol: u8, buffer: B) -> Result<ControlPacket<B>> {
        match Protocol::from(protocol) {
            ...
            Protocol::Unknown(_) => Err(Error::new(ErrorKind::InvalidData, "Unsupported")),
        }
    }
}
...
impl<B: AsRef<[u8]>> PingPacket<B> {
    pub fn new(buffer: B) -> Result<PingPacket<B>> {
        ...
        if len != 4 {
            return Err(Error::new(ErrorKind::InvalidData, "len != 4"));
        }
...
impl<B: AsRef<[u8]>> AddrPacket<B> {
    pub fn new(buffer: B) -> Result<AddrPacket<B>> {
        let len = buffer.as_ref().len();
        if len != 6 {
            return Err(Error::new(ErrorKind::InvalidData, "len != 6"));
        }
