...
impl<B: AsRef<[u8]>> NetPacket<B> {
    ...
    pub fn new_encrypt(buffer: B) -> Result<NetPacket<B>> {
    ...
    pub fn new0(data_len: usize, buffer: B) -> Result<NetPacket<B>> {
...
impl<B: AsRef<[u8]> + AsMut<[u8]>> NetPacket<B> {
    ...
    pub fn set_data_len(&mut self, data_len: usize) -> Result<()> {
        if data_len > self.buffer.as_ref().len() || data_len < 12 {
            return Err(Error::new(ErrorKind::InvalidData, "data_len invalid"));
        }
